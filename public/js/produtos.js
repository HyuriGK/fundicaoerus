(() => {
  ...
  function tabContent(data) {
    ...
    const main = `<section class="product-section"><h3><i class="fa-solid fa-cube"></i> Identificação</h3><div class="info-grid">${field('Código',p.codigo)}${field('Situação',p.situacao)}${field('Data de cadastro',date(p.data_cadastro))}${field('Nome',p.nome,'full')}${field('Apelido',p.apelido,'wide')}${field('Nome técnico',p.nome_tecnico)}${field('Cliente',p.cliente_nome,'full')}${field('Grupo',p.grupo)}${field('Subgrupo',p.subgrupo)}${field('Divisão',p.divisao)}${field('Unidade',p.unidade)}${field('Un. produção',p.unidade_producao)}${field('NCM',p.ncm)}${field('Indicador de estoque',extra.indicador_estoque,'wide')}${field('Peso líquido',p.peso_liquido ? `${num(p.peso_liquido)} kg` : null)}${field('Peso bruto',p.peso_bruto ? `${num(p.peso_bruto)} kg` : null)}${field('Peso estimado',extra.peso_estimado ? `${num(extra.peso_estimado)} kg` : null)}${field('Observação',p.observacao,'full')}</div></section>`;
    ...
    const material = m ? `<section class="product-section"><h3><i class="fa-solid fa-flask"></i> Material e processo</h3><div class="info-grid">${field('Material',m.material,'wide')}${field('Código do material',m.material_id)}${field('Lote',m.lote)}${field('Modelo',m.modelo)}${field('Processo',m.processo)}${field('Contração',m.contracao)}${field('Peso estimado',m.peso_estimado ? `${num(m.peso_estimado)} kg` : null)}${field('Dureza',m.dureza_min || m.dureza_max ? `${num(m.dureza_min)} a ${num(m.dureza_max)} HB` : null)}${field('Revisão',m.revisao)}${field('Documento',m.documento)}${field('Observação',m.observacao,'full')}</div></section><section class="product-section"><h3><i class="fa-solid fa-atom"></i> Composição química</h3><div class="chemistry">${Array.isArray(chemistry) && chemistry.length ? chemistry.map(item => field(item.elemento, item.min !== null || item.max !== null ? `${num(item.min)} — ${num(item.max)}` : null)).join('') : empty('Composição química não cadastrada.')}</div></section>` : empty('Material não cadastrado para este produto.');
    const photosPanel = photos.length ? `<div class="photo-grid">${photos.map(photo => `<div class="product-photo"><img src="${photo.imagem}" alt="Foto do produto" loading="lazy"></div>`).join('')}</div>` : empty('Não há fotos cadastradas para este produto.');
    const purchases = suppliers.length ? `<table class="product-table"><thead><tr><th>Fornecedor</th><th>Código fornecedor</th><th>Preço unitário</th><th>Principal</th><th>Atualização</th></tr></thead><tbody>${suppliers.map(row => `<tr><td>${text(row.fornecedor_nome)}</td><td>${text(row.codigo_produto_fornecedor)}</td><td>${money(row.preco_unitario)}</td><td>${row.principal ? 'Sim' : '—'}</td><td>${date(row.data_hora)}</td></tr>`).join('')}</tbody></table>` : empty('Não há fornecedores vinculados a este produto.');
    ...
    return { principal:main, precos:prices, material, fotos:photosPanel, compras:purchases, medidas:measurePanel, comercial };
  }

  function showTab(name) {
    document.querySelectorAll('.product-tabs button').forEach(button => button.classList.toggle('active', button.dataset.tab === name));
  ...
  document.querySelectorAll('.product-tabs button').forEach(button => button.addEventListener('click', () => showTab(button.dataset.tab)));
})();
