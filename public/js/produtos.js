(() => {
  ...
  function tabContent(data) {
    ...
    const weightStack = `<div class="weight-stack division-slot">${field('Peso líquido',p.peso_liquido ? `${num(p.peso_liquido)} kg` : null)}${field('Peso bruto',p.peso_bruto ? `${num(p.peso_bruto)} kg` : null)}${field('Peso estimado',extra.peso_estimado ? `${num(extra.peso_estimado)} kg` : null)}</div>`;
    const main = `<section class="product-section"><h3><i class="fa-solid fa-cube"></i> Identificação</h3><div class="info-grid">${field('Código',p.codigo)}${field('Situação',p.situacao)}${field('Data de cadastro',date(p.data_cadastro))}${field('Nome',p.nome,'full')}${field('Cliente',p.cliente_nome,'full')}${field('Grupo',p.grupo)}${field('Subgrupo',p.subgrupo)}${field('NCM',p.ncm)}${field('Divisão',p.divisao,'weight-slot')}${weightStack}${field('Observação',p.observacao,'full')}</div></section>`;
