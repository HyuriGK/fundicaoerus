    ...
    try {
        ...
        const query = `
            SELECT 
                ...
                PC.PRO_PESO_LIQUIDO_PPRC, PC.PRECO_POR_KG_PPRC,
                PP.NOME_PRODUTO_PPR as PRO_NOME_PPRC,
                P.DATA_PED as DATA_PEDIDO, C.RAZAO_SOCIAL_CLI as CLIENTE
            FROM PEDIDO_PRODUTO_CALCULO_PRECO PC
            LEFT JOIN PEDIDO P ON PC.PPR_CODIGO_PPRC = P.CODIGO_PED
            LEFT JOIN PEDIDO_PRODUTO PP ON (PC.PPR_CODIGO_PPRC = PP.CODIGO_PPR AND PC.PPR_ITEM_PPRC = PP.ITEM_PPR)
