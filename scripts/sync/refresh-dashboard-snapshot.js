...
async function refreshDashboardSnapshot() {
    ...
    const [fat, carteira, refugo, producao, meta] = await Promise.all([
        ...
        pool.query("SELECT UPPER(TRIM(COALESCE(p.data->>'NOME_CLIENTE','Desconhecido'))) cliente,SUM(GREATEST(0,COALESCE(NULLIF(REPLACE(p.data->>'QUANTIDADE_PPR',',','.'),'')::numeric,0)-COALESCE(NULLIF(REPLACE(p.data->>'QUANTIDADE_FATURADA_PPR',',','.'),'')::numeric,0))*COALESCE(NULLIF(REPLACE(p.data->>'PESO_UNIT',',','.'),'')::numeric,NULLIF(REPLACE(p.data->>'PESO_PRODUTO',',','.'),'')::numeric,0)) total FROM firebird_sync_emissoes p WHERE p.data->>'STATUS_PPR' <> 'C' GROUP BY 1 ORDER BY 2 DESC LIMIT 10"),
