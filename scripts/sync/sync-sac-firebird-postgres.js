...
const { ensureSacEmailNotificationsTable, enviarSacEmail, registrarSacNaoEnviada } = require('../../src/sac-email-notifications');
...
async function sync() {
    ...
    try {
        ...
        await ensureSacEmailNotificationsTable(client);
        const anteriores = await client.query('SELECT codigo FROM sac_firebird_sync');
        const notificacoes = await client.query('SELECT sac_codigo FROM sac_email_notifications');
        const codigosAnteriores = new Set(anteriores.rows.map(row => Number(row.codigo)));
        const notificacoesExistentes = new Set(notificacoes.rows.map(row => Number(row.sac_codigo)));
        const primeiraCarga = anteriores.rows.length === 0 && notificacoesExistentes.size === 0;
        const novosSacs = primeiraCarga ? [] : cabecalhos.filter(row => !codigosAnteriores.has(Number(row.CODIGO_SAV)) && !notificacoesExistentes.has(Number(row.CODIGO_SAV)));
        const sacPorCodigo = new Map();
        ...
        for (const row of cabecalhos) {
            ...
            sacPorCodigo.set(Number(codigo), data);
        ...
        await client.query('COMMIT');
        if (primeiraCarga) {
            for (const sac of cabecalhos) await registrarSacNaoEnviada(pool, sac, 'SAC existente antes do início do monitoramento de e-mails.');
        } else {
            for (const sac of novosSacs) await enviarSacEmail(pool, sacPorCodigo.get(Number(sac.CODIGO_SAV)) || sac);
        }
