    ...
    try {
        ...
        const query = `
            SELECT 
                ...
                (SELECT FIRST 1 E.ENTREGA_PETR FROM PEDIDO_PRODUTO_ENTREGA E WHERE E.PPR_CODIGO_PETR = P.CODIGO_PPR AND E.PPR_ANO_PETR = P.ANO_PPR AND E.PPR_ITEM_PETR = P.ITEM_PPR AND E.PPR_EMPRESA_PETR = P.EMPRESA_PPR ORDER BY E.CODIGO_PETR DESC) as ENTREGA_PETR,
                (SELECT FIRST 1 E.DATA_PETR FROM PEDIDO_PRODUTO_ENTREGA E WHERE E.PPR_CODIGO_PETR = P.CODIGO_PPR AND E.PPR_ANO_PETR = P.ANO_PPR AND E.PPR_ITEM_PETR = P.ITEM_PPR AND E.PPR_EMPRESA_PETR = P.EMPRESA_PPR ORDER BY E.CODIGO_PETR DESC) as ENTREGA_PETR_EMISSAO
        ...
        if (results.length > 0) {
            ...
            for (let i = 0; i < results.length; i += BATCH_SIZE) {
                ...
                await recordDeliveryHistory(pgClient, deduped.map(r => ({
                    ...
                    emissionDate: toDateOnly(r.ENTREGA_PETR_EMISSAO)
                })));
