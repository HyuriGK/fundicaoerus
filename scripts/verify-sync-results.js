...
(async () => {
    try {
        ...
        // Sample records
        const sampleRes = await pool.query('SELECT chave_origem, data_producao, setor, produto, op, codigo_peca, quantidade FROM producao_apontada_sincronizada WHERE op IS NOT NULL OR codigo_peca IS NOT NULL ORDER BY data_producao DESC LIMIT 5');
