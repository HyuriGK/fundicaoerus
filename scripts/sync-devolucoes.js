...
async function sincronizar() {
    console.log('🚀 Iniciando sincronização de DEVOLUÇÕES (Histórico 90 dias)...');
    ...
    try {
        ...
        const result = await new Promise((resolve, reject) => {
            db.query(query, [dataInicio], (err, result) => {
                if (err) reject(err);
                else resolve(result || []);
            });
        });
        ...
        db.detach();
        console.log('✅ Sincronização de devoluções finalizada com sucesso.');
        process.exit(0);

    } catch (error) {
        console.error('❌ Erro crítico na sincronização de devoluções:', error);
        process.exit(1);
    }
}
