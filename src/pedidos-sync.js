    ...
    try {
        ...
        const query = `
            SELECT ${monthly ? 'DISTINCT ON (EXTRACT(MONTH FROM s.snapshot_date))' : ''}
                ...
                COALESCE((TO_JSONB(s)->>'fechamento_manual_qty')::numeric, 0) AS fechamento_manual_qty,
                COALESCE((TO_JSONB(s)->>'fechamento_manual_weight')::numeric, 0) AS fechamento_manual_weight,
                COALESCE((TO_JSONB(s)->>'fechamento_manual_value')::numeric, 0) AS fechamento_manual_value,
