...
// --- ROTA GET: Carregar dados iniciais ---
router.get('/', async (req, res) => {
    ...
    try {
        ...
        console.error("Erro GET Refugo:", error);
    ...
    try {
        ...
        if (action === 'save-all') {
            ...
            for (const row of data) {
                // row[2] é a data. Precisamos validar se vem do Excel (número) ou string
                let dateVal = row[2];
        ...
        // 2. Salvar Produção Mensal (Meta/Base para cálculo de %)
        if (action === 'save-prod') {
        ...
        console.error("Erro POST Refugo:", error);
        res.status(500).json({ error: error.message });
    } finally {
        client.release();
    }
