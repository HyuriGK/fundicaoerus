...
router.get('/nfe-entrada-produtos', async (req, res) => {
    const dataInicio = /^\d{4}-\d{2}-\d{2}$/.test(req.query.dataInicio || '') ? req.query.dataInicio : new Date().toISOString().slice(0, 8) + '01';
    const dataFim = /^\d{4}-\d{2}-\d{2}$/.test(req.query.dataFim || '') ? req.query.dataFim : new Date().toISOString().slice(0, 10);
    const cfopsCte = ['135200', '135201', '135202', '235200', '235201', '235202'];
    ...
    try {
        const { rows } = await pool.query(`
            SELECT data, cnpj, prestador, nota_fiscal, compra_id, item, produto_codigo, produto,
                SUM(valor) AS valor,
                STRING_AGG(DISTINCT cfop::text, ', ') AS cfop,
                SUM(icms) AS icms,
                SUM(ipi) AS ipi,
            ...
            GROUP BY data, cnpj, prestador, nota_fiscal, compra_id, item, produto_codigo, produto
            ORDER BY data DESC, nota_fiscal DESC, item ASC
        `, [dataInicio, dataFim, cfopsCte]);
        const notas = rows.map(row => ({
            data: row.data, cnpj: String(row.cnpj || '').trim(), prestador: String(row.prestador || '').trim(),
            notaFiscal: row.nota_fiscal, item: row.item, produtoCodigo: row.produto_codigo || '', produto: row.produto || '',
            valor: Number(row.valor) || 0, cfop: row.cfop, icms: Number(row.icms) || 0, ipi: Number(row.ipi) || 0,
            pis: Number(row.pis) || 0, cofins: Number(row.cofins) || 0,
            centroCustoCodigo: row.centro_custo_codigo || '', centroCustoDescricao: row.centro_custo || 'Sem centro de custo'
        }));
        res.json({ success: true, dataInicio, dataFim, notas, total: notas.reduce((sum, nota) => sum + nota.valor, 0) });
    } catch (error) {
        console.error('[contabilidade/nfe-entrada-produtos]', error.message);
        res.status(500).json({ success: false, error: 'NÃ£o foi possÃ­vel consultar os produtos das NF-es de entrada.' });
    }
});
